...
# Part 1: Terminology and translation

## The concept-oriented model

Following the model used by TBX (ISO 30042), IATE, UNTerm, and WIPO Pearl, each entry in the system represents one concept. All language variants of that concept live together in a single file. This is the opposite of how most localization tools work (one file per language), but it's the right model for terminology because translators and reviewers need to see all languages at once.

## How terms are structured

Each term has concept-level metadata that applies regardless of language, and per-language fields that vary by translation.

**Concept-level fields:** `code`, `id`, `slug`, `project`, `status`, `domain`, `category`, `related`.

**Per-language fields (under `translations.{lang}`):** `term`, `definition`, `context`, `part_of_speech`, `aliases`, `source`, `confidence`.

Part of speech varies by language because the same concept may be a noun in English but a different grammatical category in another language. Context sentences only make sense in the target language. Source citations are per-language because the same concept may cite a different language edition of a document.

Short definitions stay in YAML frontmatter. Extended narrative content (measurement details, background, scope notes) lives in separate `{code}_description_{lang}.md` files within the term's folder. Not every term needs descriptions, and not every language needs one.

**Source attribution** uses a structured object with `text` (the human-readable citation) and an optional `url` (link to the source document). Both are per-language because the same concept may cite a completely different source for a particular language's definition.
...
The `confidence` field under each language's translation block records how reliable a given translation is, on a 1–5 integer scale:
...
## What we studied: terminology systems
...
## What we studied: git-based translation and tools
...
**Folder-based terms with code-prefixed files.** Each term lives in a directory named after its code (e.g., `terms/hips-2025/mh0301/`). The directory contains `{code}_index.md` for frontmatter and `{code}_description_{lang}.md` for extended narrative content. The code prefix on filenames prevents orphaned fragments when moving or copying files. The build pipeline discovers terms by looking for `{code}_index.md` inside each directory.

**Per-language description files.** Short definitions stay in YAML frontmatter. Extended narrative content (diagrams, measurement details, background) lives in separate description files within the term folder. This keeps the structured data clean while allowing full markdown for richer content.

**Confidence excluded from Weblate.** Confidence is reviewer metadata, not translator-editable text. Translators in Weblate should not be able to change the confidence rating of their own work — that's the reviewer's job. By excluding `confidence` from `TRANSLATABLE_FIELDS`, it never appears in the compiled Weblate JSON files. Reviewers set it by editing the YAML frontmatter directly or through CSV/JSON import.
